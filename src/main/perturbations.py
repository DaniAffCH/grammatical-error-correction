...
resources_base_path = Path(__file__).parents[0] / "resources"
...
class AddSpellingError(Perturbation):
    ...
    def perturb(cls, text: Text) -> None:
        words = word_tokenize(text.transformed)
        available_words = [
            word for word in words if word.lower() in cls.spell_error]
        if available_words:
...
class ChangeVerbForm(Perturbation):
    ...
    def perturb(cls, text: Text) -> None:
        ...
        if available_words:
            ...
            text.transformed = text.transformed.replace(
                " " + word, " " + transformed)
...
class CommonErrors(Perturbation):
    ...
    def perturb(cls, text: Text) -> None:
        ...
        words = word_tokenize(text.transformed)
        available_words = [
            word for word in words if word.lower() in cls.errors]
        if available_words:
            word = choice(available_words)
